  ...
  if (token) {
    jwt.verify(token, process.env.JWT_SECRET_KEY, async (err, decode) => {
      if (err) {
        ...
        req["user"] = decode;
        await db.connect();
        const user = await Users.findById(decode.user.user._id).populate({
          path: "company",
          select: "isBlock",
        });
        await db.disconnect();
        if (user.isBlock)
          return res.status(401).json({ message: "User profile was blocked" });
        if (user.company?.isBlock)
          return res.status(401).json({ message: "Company was blocked" });
        next();
      }
    });
  } else {
